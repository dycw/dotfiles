...
if command -v git >/dev/null 2>&1; then
    ...
    gbdr() {
        ...
        gf && git push origin --delete "${__gbdr_branch}"
    }
    gbm() { git branch -m "$1"; }
    ...
    __git_push() {
        ...
        __gp_force="$1"
        __gp_web="$2"
        __gp_current_branch "${__gp_force}" || return $?
        if [ "${__gp_web}" -eq 0 ]; then
            :
        elif [ "${__gp_web}" -eq 1 ]; then
            gw
        else
            echo_date "'__gp' accepts {0, 1} for the 'web' flag; got ${__gp_web}" && return 1
        fi
    }
...
if command -v gh >/dev/null 2>&1 || command -v glab >/dev/null 2>&1; then
    ...
    ghic() {
        ...
        elif [ "${__ghic_ghl}" = 'github' ] && [ $# -eq 3 ]; then
            gh issue create -t="$1" -l="$2" -b="$3"
        elif [ "${__ghic_ghl}" = 'gitlab' ] && [ $# -eq 1 ]; then
            glab issue create -t="$1" -d='.'
        elif [ "${__ghic_ghl}" = 'gitlab' ] && [ $# -eq 2 ]; then
            glab issue create -t="$1" -l="$2" -d='.'
        elif [ "${__ghic_ghl}" = 'gitlab' ] && [ $# -eq 3 ]; then
